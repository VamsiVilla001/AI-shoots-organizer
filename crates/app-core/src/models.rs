...
/// Where a models-bundled build puts them, relative to the front end's resource
/// directory.
pub const BUNDLED_MODELS: &str = "models";
...
/// Copies bundled models into the library's models folder, once — every
/// `.onnx` under `bundled` that is missing from `destination` or the wrong
/// size. The front end resolves where a bundle lives; a build made with
/// `tauri.models.conf.json` carries one, a default build does not.
///
