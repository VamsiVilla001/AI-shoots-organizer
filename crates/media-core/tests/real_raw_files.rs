...
fn video_sample() -> Option<PathBuf> {
    std::env::var_os("SKWAD_VIDEO_FILE")
        .map(PathBuf::from)
        .filter(|path| path.is_file())
}
...
#[test]
#[ignore = "set SKWAD_VIDEO_FILE to a real video"]
fn real_video_frame_uses_the_bounded_decoder_path() {
    let Some(path) = video_sample() else {
        eprintln!("SKWAD_VIDEO_FILE is not set to a readable file");
        return;
    };
    let ffmpeg = skwad_media_core::Ffmpeg::discover(None).expect("FFmpeg must be available");
