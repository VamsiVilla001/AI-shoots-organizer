...
pub fn suggest_person_for_cluster(cluster: &Cluster, matcher: &FaceMatcher, config: &MatcherConfig) -> Option<Match> {
...
mod tests {
    ...
    fn a_cluster_of_a_known_player_is_suggested() {
        let matcher = FaceMatcher::build(vec![(7, unit(vec![1.0, 0.0, 0.0])), (8, unit(vec![0.0, 1.0, 0.0]))]);
        let embeddings: Vec<Vec<f32>> = (0..5).map(|i| unit(vec![1.0, 0.02 * i as f32, 0.0])).collect();
    ...
    fn a_cluster_of_a_stranger_gets_no_suggestion() {
        ...
        let embeddings: Vec<Vec<f32>> = (0..5).map(|i| unit(vec![0.0, 0.02 * i as f32, 1.0])).collect();
