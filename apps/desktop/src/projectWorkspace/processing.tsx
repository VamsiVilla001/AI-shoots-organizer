...
import { TaxonomyImportButton, TaxonomyManager } from '../components/TaxonomyManager'
...
export function Processing({ projects, save, onPublished }: { projects: Project[]; save: (p: Project[]) => void; onPublished: (id: string) => void }) {
  ...
  return <>
    ...
    {tab === 'auto-tags' && <>
      <div className="pw-toolbar taxonomy-toolbar"><span className="pw-muted">Taxonomy: tags with the values your studio uses. Imported values are suggested wherever a tag is typed.</span><TaxonomyImportButton /><button onClick={() => setManagingTags(current => !current)} aria-expanded={managingTags}>{managingTags ? 'Hide tag list' : 'Manage tags'}</button></div>
      {managingTags && <div className="card taxonomy-card"><TaxonomyManager /></div>}
      {autoTagSource === null ? <><p className="pw-help">Choose a processed media collection to review people and groups found automatically by SKWAD, and tag each group with values from the taxonomy.</p><div className="pw-card-grid">{processedJobs.map(shoot => <ImportCollectionCard key={shoot.id} shoot={shoot} onOpen={() => { useUi.getState().openShoot(shoot.id, 'albums'); setAutoTagSource(shoot.id); setReviewingAutoTags(false) }} onActions={event => openSourceMenu(shoot, event)} />)}</div>{!shoots.isPending && processedJobs.length === 0 && <div className="pw-empty"><h2>No media ready for auto tagging</h2><p>Finish processing an import first. It will appear here when analysis is complete.</p></div>}</> : <><nav className="pw-breadcrumb" aria-label="Breadcrumb"><button onClick={() => { setAutoTagSource(null); setReviewingAutoTags(false) }}>Auto tags</button><span>/</span><span aria-current="page">{shoots.data?.find(shoot => shoot.id === autoTagSource)?.name ?? 'Media collection'}</span></nav><div className="pw-toolbar"><p className="pw-help">Review recognised people, name unknown groups, tag each group, and add any automatic album directly to a project collection. Tags on this collection's groups are separate from every other collection's.</p><button onClick={() => { useUi.getState().openShoot(autoTagSource, 'review'); setReviewingAutoTags(current => !current) }}>{reviewingAutoTags ? 'Back to auto tags' : 'Review face matches'}</button></div><div className="pw-existing">{reviewingAutoTags ? <ReviewScreen /> : <AlbumsScreen withTags onAddToCollection={album => void collectAlbum(album)} />}</div></>}
    </>}
